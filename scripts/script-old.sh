...
for i in {0..5}; do
  for init_method in ${init_methods[@]}; do
      nohup python train_link_prediction.py --dataset_name ${datasets[i]} --model_name $2 --num_runs 5 --gpu ${gpu} --optimizer AdamW --patience 20 --num_epochs 100 --load_best_configs --use_wandb $1 --negative_sample_strategy inductive --position_feat_dim 32 & 
    gpu_i=$((gpu_i+1))
    gpu=${gpu_choices[gpu_i%len]}
  done
done